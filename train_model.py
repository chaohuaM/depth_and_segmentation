...
def parse_argument():
    ...
    parser.add_argument('--log_dir', type=str, help='log save directory', required=False,
                        default='logs/'+datetime.datetime.strftime(datetime.datetime.now(), '%Y_%m_%d_%H_%M_%S'))
...
def train_net_weight(model, model_train, dataset_path, batch_size, lr, start_epoch, end_epoch, Cuda, dice_loss,
                     focal_loss, cls_weights, gen, gen_val, num_classes, loss_history):
    with open(os.path.join(dataset_path, "ImageSets/train.txt"), "r") as f:
        train_lines = f.readlines()
